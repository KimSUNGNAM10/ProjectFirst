...
//				return t.startsWith("H");// h로 시작하는 이름값 출력
//			}
//		}).forEach(s -> System.out.println(s));
		//람다식 employees.stream().map( t -> t.getName().filter( t -> t.startsWith("H"))
		// 	   .forEach(s -> System.out.println(s));
		...
		// 람다식  employees.stream().map(t -> t.getName()).flatMap(t -> IntStream.range(0, t,length())
